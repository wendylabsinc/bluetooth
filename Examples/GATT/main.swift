...
struct GATTExample: AsyncParsableCommand {
  ...
  @Flag(
    name: .long, help: "Require bonding when connecting as a central (used with --connect-address)."
  )
  ...
  mutating func run() async throws {
    ...
    let characteristicUUID = BluetoothUUID(
      UUID(uuidString: "7F0B8B1F-EDB4-4E1F-A04E-CE88B0E8C1A3")!)
    ...
    if pairing {
      if connectAddress == nil {
        ...
        print(
          "Pairing handling is already active for the central role; peripheral prompts will use environment defaults."
        )
      }
    }
  ...
  private static func handlePairingRequest(
    ...
    case .displayPinCode(let display):
      print(
        "Pairing (\(role)) display PIN \(display.pinCode) for \(peerLabel(central: display.central, peripheral: display.peripheral))"
      )
    case .displayPasskey(let display):
      let entered = display.entered.map { " entered=\($0)" } ?? ""
      print(
        "Pairing (\(role)) display passkey \(display.passkey)\(entered) for \(peerLabel(central: display.central, peripheral: display.peripheral))"
      )
    case .pinCode(let request):
      if let pin {
        print(
          "Pairing (\(role)) responding with PIN for \(peerLabel(central: request.central, peripheral: request.peripheral))"
        )
    ...
    case .passkey(let request):
      if let passkey {
        print(
          "Pairing (\(role)) responding with passkey for \(peerLabel(central: request.central, peripheral: request.peripheral))"
        )
    ...
    case .confirmation(let request):
      print(
        "Pairing (\(role)) confirmation requested for \(peerLabel(central: request.central, peripheral: request.peripheral))"
      )
      await request.respond(autoAccept)
    case .authorization(let request):
      print(
        "Pairing (\(role)) authorization requested for \(peerLabel(central: request.central, peripheral: request.peripheral))"
      )
      await request.respond(autoAccept)
    case .serviceAuthorization(let request):
      let service = request.serviceUUID.map(\.description) ?? "unknown"
      print(
        "Pairing (\(role)) service authorization requested (\(service)) for \(peerLabel(central: request.central, peripheral: request.peripheral))"
      )
      await request.respond(autoAccept)
    }
  }
