...
struct CentralPairingExample: AsyncParsableCommand {
    ...
    private static func handlePairingRequest(
        ...
        case .displayPinCode(let display):
            print("Pairing display PIN \(display.pinCode) for \(peerLabel(central: display.central, peripheral: display.peripheral))")
        case .displayPasskey(let display):
            let entered = display.entered.map { " entered=\($0)" } ?? ""
            print("Pairing display passkey \(display.passkey)\(entered) for \(peerLabel(central: display.central, peripheral: display.peripheral))")
        case .pinCode(let request):
            if let pin {
                print("Pairing responding with PIN for \(peerLabel(central: request.central, peripheral: request.peripheral))")
        ...
        case .passkey(let request):
            if let passkey {
                print("Pairing responding with passkey for \(peerLabel(central: request.central, peripheral: request.peripheral))")
        ...
        case .confirmation(let request):
            print("Pairing confirmation requested for \(peerLabel(central: request.central, peripheral: request.peripheral))")
            await request.respond(autoAccept)
        case .authorization(let request):
            print("Pairing authorization requested for \(peerLabel(central: request.central, peripheral: request.peripheral))")
            await request.respond(autoAccept)
        case .serviceAuthorization(let request):
            let service = request.serviceUUID.map(\.description) ?? "unknown"
            print("Pairing service authorization requested (\(service)) for \(peerLabel(central: request.central, peripheral: request.peripheral))")
            await request.respond(autoAccept)
        }
    }
