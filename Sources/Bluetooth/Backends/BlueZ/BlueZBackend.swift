  ...
  struct _BlueZCentralBackend: _CentralBackend {
    ...
    init(options: BluetoothOptions) {
      ...
      BluetoothLogger.backend.info(
        "BlueZ central backend initialized",
        metadata: [
    ...
    func startAccepting(psm: L2CAPPSM) throws -> BlueZL2CAP.Listener {
      guard var state = listeners[psm] else {
        throw BluetoothError.invalidState(
          "No L2CAP listener published for PSM 0x\(String(psm.rawValue, radix: 16))")
      }
      if state.isAccepting {
        throw BluetoothError.invalidState(
          "L2CAP incoming stream already active for PSM 0x\(String(psm.rawValue, radix: 16))")
      }
  ...
  struct _BlueZPeripheralBackend: _PeripheralBackend {
    ...
    init(options: BluetoothOptions) {
      ...
      self.client = client
      self.adapterPath = selection.path
      self.advertisingController = _BlueZAdvertisingController(
        client: client, adapterPath: selection.path)
      self.connectionEventsController = _BlueZPeripheralConnectionEventsController(
        client: client, adapterPath: selection.path)
      self.gattServerController = _BlueZGATTServerController(
        client: client, adapterPath: selection.path)
      self.agentController = _BlueZPeripheralAgentController(client: client)

      BluetoothLogger.backend.info(
        "BlueZ peripheral backend initialized",
        metadata: [
          BluetoothLogMetadata.adapterPath: "\(selection.path)"
        ])
    }
