  ...
  actor _BlueZPeripheralAgentController {
    ...
    private func handleMessage(_ message: DBusMessage, connection: DBusClient.Connection) async {
      ...
      if message.interface == "org.freedesktop.DBus.Introspectable", member == "Introspect" {
        await sendReply(
          message, connection: connection, body: [.string(Self.agentIntrospectionXML)])
        return
      }
    ...
    private func handleRequestPinCode(_ message: DBusMessage, connection: DBusClient.Connection)
      async
    {
    ...
    private func handleDisplayPinCode(_ message: DBusMessage, connection: DBusClient.Connection)
      async
    {
      let peer = peerContext(from: message)
    ...
    private func handleRequestPasskey(_ message: DBusMessage, connection: DBusClient.Connection)
      async
    {
      let peer = peerContext(from: message)
    ...
    private func handleDisplayPasskey(_ message: DBusMessage, connection: DBusClient.Connection)
      async
    {
      let peer = peerContext(from: message)
    ...
    private func handleRequestConfirmation(
      _ message: DBusMessage, connection: DBusClient.Connection
    ) async {
      let peer = peerContext(from: message)
    ...
    private func handleAuthorizeService(_ message: DBusMessage, connection: DBusClient.Connection)
      async
    {
      let peer = peerContext(from: message)
    ...
    private func awaitAuthorization(central: Central?, peripheral: Peripheral?) async -> Bool {
      ...
      return await withCheckedContinuation { continuationResult in
        ...
        let request = PairingRequest.authorization(
          PairingAuthorizationRequest(central: central, peripheral: peripheral) {
            [weak self] accepted in
            guard let self else { return }
    ...
    private func registerAgent(_ connection: DBusClient.Connection) async throws {
      ...
      if let reply = try await connection.send(defaultRequest), reply.messageType == .error {
        ...
        if name != "org.bluez.Error.AlreadyExists" {
          logger.error(
            "RequestDefaultAgent failed", metadata: [BluetoothLogMetadata.error: "\(name)"])
    ...
    private func unregisterAgent(_ connection: DBusClient.Connection) async {
      ...
      do {
        if let reply = try await connection.send(request), reply.messageType == .error {
          let name = client.dbusErrorName(reply) ?? "org.freedesktop.DBus.Error.Failed"
          logger.warning(
            "UnregisterAgent failed", metadata: [BluetoothLogMetadata.error: "\(name)"])
        } else {
    ...
    private func deviceAddress(from path: String) -> BluetoothAddress? {
      ...
      let addr = component.dropFirst("dev_".count).replacingOccurrences(of: "_", with: ":")
        .lowercased()
    ...
    private func sendError(
      ...
      do {
        ...
        logger.warning(
          "Agent error reply failed", metadata: [BluetoothLogMetadata.error: "\(error)"])
      }
    }
    ...
    private struct AgentConfig: Sendable {
      ...
      static func load() -> AgentConfig {
        ...
        if let capabilityValue, !capabilityValue.isEmpty, normalized != capabilityValue {
          logger.warning(
            "Unknown agent capability, using default",
            metadata: [
