...
actor _BlueZGATTServerController {
    ...
    private func removePreparedWrites(for removedPaths: Set<String>) {
        ...
        pendingExecuteByCentral = pendingExecuteByCentral.filter { preparedWriteIDsByCentral[$0] != nil }
    }
    ...
    private func emitInterfacesAdded(
        ...
        objects.append(InterfacesAddedPayload(path: serviceState.path, interfaces: await serviceInterfaces(serviceState)))

        for charState in serviceState.characteristics {
            objects.append(InterfacesAddedPayload(path: charState.path, interfaces: await characteristicInterfaces(charState)))
            for descState in charState.descriptors {
                objects.append(InterfacesAddedPayload(path: descState.path, interfaces: await descriptorInterfaces(descState)))
            }
        }
    ...
    private func makeCharacteristicObject(_ state: CharacteristicState) -> DBusObjectServer.ExportedObject {
        ...
        let properties = [
            DBusObjectServer.Property(name: "UUID", value: .string(state.characteristic.uuid.description)),
    ...
    private func makeDescriptorObject(_ state: DescriptorState) -> DBusObjectServer.ExportedObject {
        ...
        let properties = [
            ...
            DBusObjectServer.Property(name: "Characteristic", value: .objectPath(state.characteristicPath)),
            DBusObjectServer.Property(
                name: "Flags",
                signature: "as",
                access: .read,
    ...
    private func handleStartNotify(path: String) async throws {
        ...
        guard state.characteristic.properties.contains(.notify)
        ...
        if let continuation = requestContinuation {
            let type: GATTClientSubscriptionType = state.characteristic.properties.contains(.notify) ? .notification : .indication
            let subscription = GATTSubscription(central: nil, characteristic: state.characteristic, type: type)
    ...
    private func supportsWrite(flags: [String]) -> Bool {
        flags.contains("write") || flags.contains("write-without-response") || flags.contains("encrypt-write")
    }
    ...
    private func commitPreparedWrites(for centralKey: String, shouldCommit: Bool) async {
        ...
        for entry in entries {
            ...
            case .characteristic(let path):
                ...
                guard let updated = try? applyingWrite(entry.data, to: state.value, offset: entry.offset) else {
                    continue
                }
                state.value = updated
                characteristicStates[path] = state
    ...
    private func characteristicInterfaces(_ state: CharacteristicState) async -> [DBusValue: DBusValue] {
