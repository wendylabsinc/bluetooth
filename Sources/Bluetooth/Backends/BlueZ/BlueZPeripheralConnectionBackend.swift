...
actor _BlueZPeripheralConnectionBackend: _PeripheralConnectionBackend {
    ...
    func notifications(
        ...
        return AsyncThrowingStream { continuation in
            let id = UUID()
            var state = notificationStates[path] ?? NotificationState(type: initialType, continuations: [:])
    ...
    private func loadGattObjects(_ connection: DBusClient.Connection) async throws -> GattObjectCache {
        ...
        for (pathValue, interfacesValue) in objects {
            ...
            guard let props = properties(for: "org.bluez.GattService1", in: interfaces) else { continue }
        ...
        for (pathValue, interfacesValue) in objects {
            ...
            guard path.hasPrefix(devicePath) else { continue }
            guard case .dictionary(let interfaces) = interfacesValue else { continue }
            guard let props = properties(for: "org.bluez.GattCharacteristic1", in: interfaces) else { continue }
        ...
        for (pathValue, interfacesValue) in objects {
            ...
            guard path.hasPrefix(devicePath) else { continue }
            guard case .dictionary(let interfaces) = interfacesValue else { continue }
            guard let props = properties(for: "org.bluez.GattDescriptor1", in: interfaces) else { continue }
    ...
    private func resolveCharacteristicPath(_ characteristic: GATTCharacteristic, in cache: GattObjectCache) -> String? {
        ...
        if characteristic.instanceID == nil {
            return cache.characteristicsByPath.first(where: {
                $0.value.uuid == characteristic.uuid && $0.value.service.uuid == characteristic.service.uuid
            })?.key
        }
        return nil
    }
    ...
    private func resolveDescriptorPath(_ descriptor: GATTDescriptor, in cache: GattObjectCache) -> String? {
