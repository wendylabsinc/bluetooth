    ...
    init(_ cbuuid: CBUUID) {
      ...
      if uuidString.count == 4 {
        ...
        guard let value = UInt16(uuidString, radix: 16) else {
          ...
          self = .bit128(
            UUID(uuidString: "0000\(uuidString)-0000-1000-8000-00805F9B34FB") ?? UUID())
          return
        }
        self = .bit16(value)
  ...
  extension GATTCharacteristicProperties {
    ...
    init(_ cbProperties: CBCharacteristicProperties) {
      ...
      if cbProperties.contains(.authenticatedSignedWrites) {
  ...
  struct _CoreBluetoothCentralBackend: _CentralBackend {
    ...
    func removeBond(for peripheral: Peripheral) async throws {
      throw BluetoothError.unimplemented(
        "Bond removal not available in CoreBluetooth - use system Settings")
    }
  ...
  private actor _CoreBluetoothCentralController {
    ...
    private var pendingConnections:
      [UUID: CheckedContinuation<any _PeripheralConnectionBackend, Error>] = [:]
    ...
    private func setupDelegateCallbacks() {
      ...
      delegate.onDiscover = { [weak self] peripheral, advertisementData, rssi in
        guard let self else { return }
        let advData = SendableAdvertisementData(
    ...
    private func handleDiscovery(
      _ peripheral: CBPeripheral, _ sendableAdvData: SendableAdvertisementData, _ rssi: Int
    ) {
      ...
      let serviceData = sendableAdvData.serviceData.reduce(into: [BluetoothUUID: Data]()) {
        result, pair in
        if pair.key.count == 4 {
      ...
      if let mfgData = sendableAdvData.manufacturerData, mfgData.count >= 2 {
        ...
        manufacturerData = ManufacturerData(
          companyIdentifier: companyId, data: mfgData.dropFirst(2))
      }
    ...
    private func handleConnect(_ peripheral: CBPeripheral) {
      if let continuation = pendingConnections.removeValue(forKey: peripheral.identifier) {
        let connectionBackend = _CoreBluetoothPeripheralConnectionBackend(
          peripheral: peripheral, centralManager: centralManager)
    ...
    private func handleDisconnect(_ peripheralId: UUID) {
      if let continuation = pendingConnections.removeValue(forKey: peripheralId) {
        continuation.resume(
          throwing: BluetoothError.connectionFailed("Peripheral disconnected during connection"))
      }
    ...
    func scan(
      ...
      let serviceUUIDs: [CBUUID]? =
        filter?.serviceUUIDs.isEmpty == false
    ...
    private func handleConnectionTimeout(uuid: UUID, peripheral: CBPeripheral) {
      if let continuation = pendingConnections.removeValue(forKey: uuid) {
        ...
        continuation.resume(
          throwing: BluetoothError.connectionFailed("Connection timed out after 30 seconds"))
      }
    }
  ...
  actor _CoreBluetoothPeripheralConnectionBackend: _PeripheralConnectionBackend {
    ...
    private var notificationContinuations:
      [CBUUID: AsyncThrowingStream<GATTNotification, Error>.Continuation] = [:]
    ...
    private func handleCharacteristicValueUpdated(
      _ uuid: CBUUID, _ value: Data, _ isIndicate: Bool, _ error: Error?
    ) {
      ...
      if pendingReads.contains(uuid), var continuations = readValueContinuations[uuid],
      ...
      if let continuation = notificationContinuations[uuid] {
        if let error {
          ...
          let notification: GATTNotification =
            isIndicate ? .indication(value) : .notification(value)
    ...
    private func handleCharacteristicWritten(_ uuid: CBUUID, _ error: Error?) {
      guard var continuations = writeValueContinuations[uuid], !continuations.isEmpty else {
        return
      }
    ...
    private func handleDescriptorValueUpdated(_ descriptor: CBDescriptor, _ error: Error?) {
      ...
      guard var continuations = readDescriptorContinuations[key], !continuations.isEmpty else {
        return
      }
    ...
    private func handleDescriptorWritten(_ descriptor: CBDescriptor, _ error: Error?) {
      let key = descriptorKey(descriptor)
      guard var continuations = writeDescriptorContinuations[key], !continuations.isEmpty else {
        return
      }
    ...
    private func handleL2CAPChannelOpened(_ wrappedChannel: SendableL2CAPChannel?, _ error: Error?)
    {
      if let error {
        ...
        l2capChannelContinuation?.resume(
          throwing: BluetoothError.l2capChannelError("Failed to open L2CAP channel"))
      }
      l2capChannelContinuation = nil
    }
    ...
    func readValue(for descriptor: GATTDescriptor) async throws -> Data {
      guard let cbChar = discoveredCharacteristics[descriptor.characteristic.uuid.cbuuid],
        let cbDescriptor = cbChar.descriptors?.first(where: { $0.uuid == descriptor.uuid.cbuuid })
      else {
    ...
    func updateConnectionParameters(_ parameters: ConnectionParameters) async throws {
      throw BluetoothError.unimplemented(
        "Connection parameter updates not available in CoreBluetooth")
    }
  ...
  struct _CoreBluetoothPeripheralBackend: _PeripheralBackend {
    ...
    func startAdvertising(
      ...
      try await controller.startAdvertising(
        advertisingData: advertisingData, scanResponseData: scanResponseData, parameters: parameters
      )
    }
    ...
    func disconnect(_ central: Central) async throws {
      throw BluetoothError.unimplemented(
        "Direct disconnect not available in CoreBluetooth peripheral role")
    }
  ...
  private actor _CoreBluetoothPeripheralController {
    ...
    private let cachedState: Mutex<BluetoothState>
    private var stateUpdatesContinuation: AsyncStream<BluetoothState>.Continuation?
    private var connectionEventsContinuation:
      AsyncThrowingStream<PeripheralConnectionEvent, Error>.Continuation?
    private var gattRequestsContinuation:
      AsyncThrowingStream<GATTServerRequest, Error>.Continuation?
    ...
    private var publishedL2CAPChannels:
      [UInt16: AsyncThrowingStream<any L2CAPChannel, Error>.Continuation] = [:]
    ...
    private func handleServiceAdded(_ wrappedService: SendableService?, _ error: Error?) {
      ...
      if let error {
        ...
        let registration = GATTServiceRegistration(
          service: gattService, characteristics: characteristics)
        continuation.resume(returning: registration)
      } else {
        continuation.resume(
          throwing: BluetoothError.serviceRegistrationFailed("Failed to add service"))
      }
      ...
      try await withCheckedThrowingContinuation {
        ...
        runLoopThread.perform {
          ...
          guard status == .open else {
            ...
            continuation.resume(
              throwing: BluetoothError.l2capChannelError(
                "Output stream not open (status=\(status.rawValue))"))
            return
          }

          let bytesWritten = data.withUnsafeBytes { buffer in
            self.outputStream.write(
              buffer.bindMemory(to: UInt8.self).baseAddress!, maxLength: data.count)
          }
          ...
          if bytesWritten < 0 {
            ...
            continuation.resume(
              throwing: BluetoothError.l2capChannelError("Failed to write to L2CAP channel"))
          } else if bytesWritten == 0 {
            self.logger.debug("Wrote 0 bytes - stream may not be ready")
            continuation.resume(
              throwing: BluetoothError.l2capChannelError("Wrote 0 bytes - stream may not be ready"))
          } else {
            self.logger.trace(
              "Write succeeded",
              metadata: [
